...
- 시드 데모 페르소나는 `user_id = 1` (**살아있는 카드 5건**, 거래 391건) — V900이 카드 5건·거래 355건,
  V901이 혜택 추천 시연용으로 카드 4건·거래 36건을 더하고, **V904가 그 4건을 소프트 삭제**한다.
  거래 391건은 그대로 남으므로 `is_deleted`를 거르지 않는 쿼리에는 계속 잡힌다
