...
-- =========================================================
-- v26: 혜택 원문 수집(크롤링) staging
-- =========================================================

DROP TABLE IF EXISTS crawl_raw_card;
-- v26: 카드사 웹페이지에서 수집한 혜택 원문 텍스트의 적재 테이블.
--
-- 이 테이블은 "수집"과 "해석"을 분리하기 위해 존재한다. 원문을 여기 보관해 두면 파서나
-- LLM 프롬프트를 고칠 때 카드사 서버를 다시 치지 않고 추출만 몇 번이든 재실행할 수 있다.
-- 반대로 이 테이블이 없으면 프롬프트를 튜닝할 때마다 61개 페이지를 다시 긁어야 한다.
--
-- benefit_* 테이블들과 달리 여기엔 해석된 값이 없다. 원문 문자열 그대로다. 해석 결과는
-- 다음 단계에서 benefit_service/benefit_tier/benefit_limit 으로 들어간다.
--
-- 카드 식별자로 우리 쪽 card_product_id 가 아니라 카드사가 쓰는 external_card_code 를
-- 쓴다. 수집 시점엔 그 카드가 card_product 에 있는지 아직 모르고(신규 카드일 수 있고),
-- 매칭 자체가 다음 단계의 일이기 때문이다. 그래서 card_product FK 를 걸지 않는다.
CREATE TABLE crawl_raw_card (
    ...
    -- 원문의 어느 영역인지. KB 상세 페이지의 탭 구조에 대응한다
    -- (SUMMARY=주요혜택, DETAIL=상세혜택, ANNUAL_FEE=연회비).
    -- 카드당 한 행이 아니라 섹션당 한 행인 이유: 영역마다 성격이 달라 다음 단계에서
    -- 서로 다른 프롬프트로 처리하고, 한 영역만 바뀌었을 때 그 영역만 재처리하기 위함이다.
    -- KB의 '확인사항' 탭은 수집하지 않는다 — 카드당 8천자가 넘는데 내용이 할인 제외매출/
    -- 연체이자율 같은 법적 고지문이라(카드 간 유사도 52%) 혜택 판정에 쓸 값이 없고,
    -- 넣으면 오히려 "상품권 구입은 할인 제외" 같은 문장이 혜택으로 오추출된다.
