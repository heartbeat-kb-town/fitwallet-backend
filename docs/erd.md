...
> 시드에 든 카드사는 **신한 · 현대 · KB국민 3사**입니다. 이 3사가 `brand` 등록 요건([§2.3](#brand--체인브랜드))의 "근거" 기준이기도 합니다 — 셋 중 어느 곳의 혜택도 지목하지 않는 브랜드는 등록하지 않습니다.
>
> **왜 이 3사인지는 팀 노션 "고도화 문서"의 [지원 카드사 범위 확정](https://app.notion.com/p/3b8a561881a481ef822cfbe0a3bdb564) 문서에 있습니다.** 카드사별 `robots.txt` 실측과 제외 근거는 스키마가 아니라 수집 정책이라 그쪽이 정본입니다.
...
> **`brand` 1행 = 실제 사업 브랜드 1개입니다.** 표기 변형은 `brand` 행이 아니라 `brand_alias` 행으로 갑니다.
>
> **어떤 상표를 `brand`로 등록하는지의 판단 기준은 팀 노션 "고도화 문서"의 [brand 판단 기준과 별칭 규칙](https://app.notion.com/p/3b8a561881a4816598c5ecaad3c92d6e) 문서에 있습니다.** 스키마가 아니라 기준정보 정책이라 그쪽이 정본입니다.

시드는 32건이고 업종 분포는 카페/디저트 11 · 편의점/마트 12 · 쇼핑 3 · 푸드 2 · **병원 0** · 주유 4입니다. 병원이 0건인 것은 정상입니다 — 병원 혜택은 특정 체인을 지목하지 않는 `INDUSTRY` 스코프뿐입니다.
...
> ⚠️ **시드가 비어 있습니다.** 실제 별칭은 공공데이터 상호명이 어떤 표기로 들어오는지 확인한 뒤 채웁니다. 별칭을 짓는 규칙과 상호명 매칭 순서는 위 노션 문서에 있습니다.
>
