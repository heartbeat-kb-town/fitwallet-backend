 ...
 * 읽기 측정 유저.
 *
 * ⚠️ **활성 3만 명은 연속 블록이 아니라 10만 명 전체에 흩어져 있다.** 실측으로 확인했다 —
 * perf000100 · perf030001 · perf100000은 거래가 있고, perf000001 · perf000002 · perf000010 ·
 * perf001000 · perf010000 · perf029999 · perf050000은 전부 0건이다.
 *
 * 적재 문서의 "유저당 거래 수 p50 180 / 평균 180.0 / 최대 180"은 **활성 유저 안에서의 분포**이지
 * 전체 10만 명의 분포가 아니다. 앞번호를 아무거나 고르면 70% 확률로 거래 0건인 유저를 잡고,
 * 집계 API가 전부 빈 응답을 20ms에 돌려주면서 **"리포트가 빠르다"는 완전히 틀린 표가 나온다.**
 * 실제로 처음 perf000001로 돌렸다가 report 계열이 22~32ms로 찍혀 이 함정을 밟았다.
 *
 * 그래서 기본값을 실측으로 확인한 활성 유저로 두고, setup()에서 한 번 더 검증한다.
 */
const READ_USER = __ENV.READ_USER || 'perf000100';
/** 변경 측정 전용 유저. 읽기 유저와 반드시 달라야 READ 표가 오염되지 않는다. */
const WRITE_USER = __ENV.WRITE_USER || 'perf099999';
...
export function setup() {
     ...
     *
     * 이 검사가 없으면 비활성 유저(10만 중 7만)로 돌려도 테스트가 초록불로 끝나고,
    ...
    if (categories.length === 0 && received === 0) {
        throw new Error(
            ...
            + '활성 3만 명은 10만 명에 흩어져 있어 앞번호가 활성이라는 보장이 없다. '
            + 'READ_USER 환경변수로 활성 유저를 지정해라 — 실측 확인된 값: perf000100 / perf030001 / perf100000.');
    }
