...
/** 브랜드 범위 혜택 서비스 하나와 브랜드 하나를 조합한 월간 혜택 행. */
@ApiModel(description = "브랜드별 카드 월간 혜택")
@Getter
@NoArgsConstructor
@AllArgsConstructor
@Builder
public class CardMonthlyBrandBenefitResponse {

    @ApiModelProperty(value = "혜택 서비스 ID", example = "53")
    private Long benefitServiceId;

    @ApiModelProperty(value = "브랜드 ID", example = "15")
    private Long brandId;

    @ApiModelProperty(value = "브랜드명", example = "GS25")
    private String brandName;

    @ApiModelProperty(value = "브랜드 이미지 URL. 이미지가 없으면 null")
    private String brandImageUrl;

    @ApiModelProperty(value = "UI 행 제목. 브랜드명과 동일하다", example = "GS25")
    ...
    /** 현재 혜택 서비스가 실제 적용되고 현재 브랜드에 해당하는 이번 달 거래 건수. */
    @ApiModelProperty(value = "현재 혜택이 실제 적용된 현재 브랜드의 거래 건수", example = "1")
    private long transactionCount;

    /** {@link #transactionCount}와 동일한 거래 집합의 할인 전 결제금액 합계. */
    @ApiModelProperty(value = "현재 혜택이 실제 적용된 현재 브랜드 거래의 결제금액 합계", example = "23800")
