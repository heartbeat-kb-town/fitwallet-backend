...
class StoreControllerTest {
    ...
    void 개별_삭제_정상_호출이_200과_SEARCH_HISTORY_DELETED를_반환한다() throws Exception {
        ...
        mockMvc.perform(delete("/api/store/keywords/recent/10").header("Authorization", "Bearer access-token"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
    ...
    void 개별_삭제시_서비스가_SEARCH_HISTORY_NOT_FOUND를_던지면_404를_반환한다() throws Exception {
        ...
        mockMvc.perform(delete("/api/store/keywords/recent/999").header("Authorization", "Bearer access-token"))
    ...
    void 전체_삭제_정상_호출이_200과_SEARCH_HISTORY_ALL_DELETED를_반환한다() throws Exception {
        ...
        mockMvc.perform(delete("/api/store/keywords/recent").header("Authorization", "Bearer access-token"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
    ...
    void searchHistoryId가_숫자가_아니면_500이_아니라_400을_반환한다() throws Exception {
        ...
        mockMvc.perform(delete("/api/store/keywords/recent/abc").header("Authorization", "Bearer access-token"))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.code").value("INVALID_INPUT_VALUE"));
